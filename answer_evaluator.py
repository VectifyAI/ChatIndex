"""
Simple Answer Evaluator for ChatIndex

A lightweight answer evaluation module that uses Claude API to evaluate
if generated answers match standard answers. Designed specifically for
ChatIndex benchmarking without memU dependencies.

This module is functionally equivalent to EvaluateAgent from memU-experiment,
but uses Claude API instead of OpenAI. All methods, prompts, parsing logic,
and return formats are identical to ensure consistent evaluation results and
easy swapping between evaluators.

Available methods (matching EvaluateAgent):
- evaluate_answer_accuracy: Evaluate if a generated answer matches the standard answer
- evaluate_retrieved_events: Evaluate each retrieved event for relevance
- analyze_answer_errors: Analyze why a generated answer is incorrect
- comprehensive_evaluation: Perform comprehensive evaluation with all components
- execute_tool: Execute a specific evaluation tool by name
- execute: Execute user message with function calling support
- get_available_tools: Get list of available evaluation tools
"""
...
# Set up logger equivalent to EvaluateAgent
logger = logging.getLogger(__name__)

# Configure logging if not already configured (equivalent to setup_logging)
...
class AnswerEvaluator:
    """
    Simple answer evaluator using Claude API.
    
    Evaluates if a generated answer matches a standard answer for LoCoMo benchmark.
    """
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "claude-haiku-4-5-20251001"):
        ...
        Args:
            ...
            model: Claude model to use for evaluation (default: claude-haiku-4-5-20251001)
        """
    ...
    def evaluate_answer_accuracy(
        ...
        """
        Evaluate if a generated answer matches the standard answer.
        
        Args:
            question: The original question
            generated_answer: The AI-generated answer to evaluate
            standard_answer: The reference/standard answer to compare against
        ...
        Returns:
            ...
            - explanation: str
            - evaluation_text: str
            - question, generated_answer, standard_answer: original inputs
        """
        try:
            # System prompt (exact match with EvaluateAgent)
            system_prompt = """
            You are an expert grader that determines if answers to questions match a gold standard answer
            """.strip()
            
            # Accuracy evaluation prompt (exact match with EvaluateAgent)
            accuracy_prompt = f"""
...
"""
    ...
    def get_available_tools(self) -> List[Dict[str, Any]]:
        """Get list of available evaluation tools (exact match with EvaluateAgent)"""
        return [
            {
                "type": "function",
                "function": {
    ...
    def evaluate_retrieved_events(self, question: str, standard_answer: str, retrieved_events: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Evaluate each retrieved event for relevance to the standard answer (exact match with EvaluateAgent)"""
        try:
            ...
            for i, event_data in enumerate(retrieved_events):
                try:
                    ...
                    # Create evaluation prompt for this event (exact match with EvaluateAgent)
...
Explanation: [Your reasoning]"""
                    ...
                    response = self.client.messages.create(
                        ...
                        temperature=0.1,
                        messages=[
                            {"role": "user", "content": event_evaluation_prompt}
                        ]
                    )
                    ...
                    # Parse the evaluation result (exact match with EvaluateAgent)
                    is_relevant = False
                    explanation = evaluation_text
    ...
    def analyze_answer_errors(self, question: str, generated_answer: str, standard_answer: str, retrieved_events: List[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Analyze why a generated answer is incorrect and provide detailed reasoning (exact match with EvaluateAgent)"""
        try:
            # Prepare retrieved events context (exact match with EvaluateAgent)
            events_context = ""
            if retrieved_events:
            ...
            # Create error analysis prompt (exact match with EvaluateAgent)
...
Recommendations: [How could the answer be improved?]"""
            ...
            # Parse the analysis result (exact match with EvaluateAgent)
            parsed_analysis = {}
    ...
    def comprehensive_evaluation(self, question: str, generated_answer: str, standard_answer: str, retrieved_events: List[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Perform comprehensive evaluation including answer accuracy, event relevance, and error analysis (exact match with EvaluateAgent)"""
        try:
            ...
            # Compile comprehensive report (exact match with EvaluateAgent)
            return {
                "success": True,
                "question": question,
                "generated_answer": generated_answer,
    ...
    def execute_tool(self, tool_name: str, **kwargs) -> Dict[str, Any]:
        """Execute a specific evaluation tool by name (exact match with EvaluateAgent)"""
        tool_methods = {
    ...
    def execute(self, user_message: str, max_iterations: int = 10) -> Dict[str, Any]:
        ...
        Note: This method uses Claude's function calling format which differs from OpenAI's.
        For full compatibility, you may need to adapt the function calling format.
        """
        try:
            ...
            while iteration < max_iterations:
                ...
                # Extract content and tool calls from response (Claude format)
                content_blocks = []
                tool_calls = []
                ...
                # Add assistant message (Claude format - content is list of blocks)
                assistant_message = {
                    "role": "assistant",
                    "content": content_blocks + tool_calls  # Mix of text and tool_use blocks
                }
                ...
                if tool_calls:
                    ...
                    for tool_call in tool_calls:
                        try:
                            ...
                            arguments = tool_call.input  # Already a dict in Claude format
                            
                            # Execute tool
                            result = self.execute_tool(tool_name, **arguments)
                            
                            # Add tool result (Claude format)
                            tool_results.append({
                                "type": "tool_result",
                    ...
                    # Add all tool results as a single user message (Claude format)
                    if tool_results:
                        messages.append({
                            "role": "user",
                ...
                # Extract final content for return value
                content = ""
